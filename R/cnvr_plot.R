...
#' @param refgene referece gene list, use for plot gene
...
#' If given clean_cnv file, will plot all CNVRs which are passed common threhold.
#' @export cnvr_plot
#'
#' @examples
cnvr_plot <- function(cnvr, assembly = "ARS", legend_x = 127, legend_y = 30, clean_cnv = NULL, sample_size = NULL, common_cnv_threshold = 0.05, refgene = "ARS") {
  if (is.null(clean_cnv)) {
    ...
    for (i in 1:nrow(high_freq)) {
      ...
      cnv_visual(clean_cnv = clean_cnv, chr_id = high_freq$Chr[i], start_position = high_freq$Start[i]/1000000, end_position = high_freq$End[i]/1000000, plot_gene = 1, refgene = refgene)
    }
  }
}
