...
plot_cnv_intensity <- function(cnvr, cnv_annotation, intensity, map, sample_size, common_cnv_threshold = 0.05) {
  ...
  for (i in 1:nrow(high_freq)){
    ...
    b <- high_freq$title[i]
    dir <- paste( b, ".png", sep = "")
    png(dir, res = 300, width = 2000, height = 3000, bg = "transparent")
    #plot_grid(zoom,zoom2, maf, baf, lrr, align = "v", axis = "t", ncol = 1, rel_heights = c(1,1,1,1,1))
    final_plot <- plot_grid(zoom_plot, baf, lrr, align = "v", axis = "t", ncol = 1, rel_heights = c(1, 1,1))
    print(final_plot)
    dev.off()
