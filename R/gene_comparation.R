...
compare_gene <- function(gene_freq_1, gene_freq_2, gene_freq_3 = NULL, common_gene_threshold = 3, title_1 = "genelist_1", title_2 = "genelist_2", title_3 = "genelist_3"){
  if (is.null(gene_freq_3)){
    ...
    colnames(ars_gene) <- paste(colnames(ars_gene), "list_1", sep = "_")

    umd_gene <- fread(gene_freq_2)
    colnames(umd_gene) <- paste(colnames(umd_gene), "list_2", sep = "_")

    ars_umd_gene <- merge(ars_gene, umd_gene, by.x = "name2_list_1", by.y = "name2_list_2", all = TRUE) #combine all gene together between two version of results
    ...
    for (i in 1:nrow(ars_umd_gene)) {
      if (ars_umd_gene$Frequency_list_1[i] < common_gene_threshold & ars_umd_gene$Frequency_list_2[i] < common_gene_threshold) {
        ars_umd_gene$Common_Gene[i] = "Common_Low"
      } else if (ars_umd_gene$Frequency_list_1[i] < common_gene_threshold & ars_umd_gene$Frequency_list_2[i] >= common_gene_threshold) {
    ...
    png(filename = "two_genelists_comparison.png", res = 300, width = 2300, height = 1700)
    compare_plot <- ggplot(data = ars_umd_gene, aes(x = Frequency_list_1, y = Frequency_list_2, color = Common_Gene)) +
      geom_point(pch = 19, size = 3, position = position_jitter(height = 0.15, width = 0.15)) +
      geom_text_repel(data = subset(ars_umd_gene, Common_Gene == "Common_High"), aes(label = name2_list_1), nudge_x = 0.3) +
      theme_classic() +
    ...
    colnames(part_gene) <- paste(colnames(part_gene), "list_3", sep = "_")

    #merge gene lists
    print("Starting merging gene lists...")
    two_gene <- merge(ars_gene, umd_gene, by.x = "name2_list_1", by.y = "name2_list_2", all = TRUE)
    three_gene <- merge(two_gene, part_gene, by.x = "name2_list_1", by.y = "name2_list_3", all = TRUE)
    ...
    for (i in 1:nrow(three_gene)) {
      if (three_gene$Frequency_list_2[i] < common_gene_threshold & three_gene$Frequency_list_1[i] < common_gene_threshold & three_gene$Frequency_list_3[i] < common_gene_threshold) {
        three_gene$Common_check[i] = "Common_low"
      }
      else if (three_gene$Frequency_list_2[i] >= common_gene_threshold & three_gene$Frequency_list_1[i] >= common_gene_threshold & three_gene$Frequency_list_3[i] >= common_gene_threshold) {
    ...
    print(subset(three_gene[three_gene$Common_check == "Common_high", ],
                 select = c("name2_list_1", "Frequency_list_1","Frequency_list_2", "Frequency_list_3", "Common_check"))) #here only plot selected columns
    print("Brief summary:")
    ...
    gene_plot <- scatterplot3d::scatterplot3d(z = three_gene$Frequency_list_3, x = three_gene$Frequency_list_1, y = three_gene$Frequency_list_2,
                                 zlab = title_3, xlab = title_1, ylab = title_2,
                                 color = color_custom, pch = 16)
    common_high <- subset(three_gene, Common_check == "Common_high", select = c("name2_list_1", "Frequency_list_1","Frequency_list_2", "Frequency_list_3", "Common_check"))#add common high gene name in plot
