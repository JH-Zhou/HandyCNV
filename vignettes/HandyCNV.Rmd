...
# Introduction
This package was originally designed for the Post-analysis of CNV results inferred from PennCNV and CNVPartition (GenomeStudio). But now it was expanded to summary and visualize ROH results and accept the input files with standard formats for the wider application. Our motivation is to provide a Standard, Reproducible and Time-saving pipelines for the post-analysis of CNV and ROH from SNP genotyping data for the most of Species. All functions can be divide to five sectors: Conversion, Summarization, Annotation, Comparison and Visualization. The most useful features are it could integrate summarize results, generate CNVR, annotated gene, plot CNVR distribution map and customize visualizing CNV and ROH with gene and other related information together. It support user to create a new folder to save results from each function, and set the title or adjust the size of final figures to meet the need of users. This package is suitable for the post-CNV analysis to not only Bovine (Cattle) but also Human and Swine (Pig) studies. As following, We will present how to use these functions to solve the problems.

# Prerequisite
To use this package, we need to install R in your computer at first (https://www.r-project.org/).<br> Then install the 'HandyCNV' from Github repository, we assuming you have some experience with R language and already have R or R Studio installed in your computer, now, let's install the package 'HandyCNV' from Github repository.
```{r}
#library(remotes)
#install_github(repo = "JH-Zhou/HandyCNV", auth_token = "3d2ac98e4c297bab332f1e68b3b2d49f3a17d6aa")
```
Then loading 'HandyCNV' and use it later.
```{r setup}
library(HandyCNV)
```
To start play with this package, we need to prepare at least a CNV result list. With only CNV results as input file, we can explore most of these functions. But to get more interested information, we need to prepare more input files, including map file, signal intensity file, pedigree, and Plink format (bed, bim and fam) files. Don't afraid to prepare these files, most of these files we already had and used in the CNV inferring progress. In order to get more valuable results, it worth to spend some time to lay the foundation, and then, you will found the rest of work are easier and will saving a lot of time. We will introduce how to prepare each input file during the relative chunk while we using it.

# Demo data
We provide some internal demo data and it should installed with package together, and we are going to use these data as an example to present how to use this package.<br> We can extract the demo data by 'system.file' (see as below). In the demo data, the first CNV results is default output format from PennCNV with ARS-UCD 1.2 (ARS) reference genome, the second CNV results is defaults output from CNVPartition with UMD 3.1 (UMD) reference genome, the third CNV list is the standard input file which could from other tools but require to prepare it by ourself. <br> Note: If you are dislike to work with this demo data, just skip this step to the section 1 and have fun with your own data.
```{r}
...
As above we mentioned, the functions in the package are five sectors: Conversion, Summarization, Annotation, Comparison and Visualization. Conversion part, could convert the map files between the maps file (default and target) provide by users. This is useful when we are detecting CNVs with two versions of reference genome, the converted map files has the well-prepared formats can used in Plink, PennCNV and our package 'HandyCNV'. Another feature is it support to convert coordinates for intervals (CNV, CNVR, QTL et al) between the defaults and target maps, which might useful when the users want to compare the results with others. Summarization part including format and details summary of CNV and ROH, make CNV summary plot, generate CNVRs and plot CNVR distribution map. Annotation part could auto-download reference gene lists from UCSC website and annotate genes for CNV, CNVR, ROH or other intervals. Comparison section including comparison of CNVs, CNVRs, Gene Frequent Lists and any other Intervals, the highlight is comparison of CNVs and CNVRs will reports details comparison results on both individual and population levels. Visualization segment contains functions to support users to customize CNV and ROH plot by Chromosome, specific Sample, any interested regions or target genes (Not for ROH right now), we can also and add more information on each plots, such as annotated gene, Log R Ratio, B Allele Frequency, SNP genotype, LD or the source of CNVs. Besides, its can plotting genes separately from reference gene lists which might useful when we want to combine with other plots.
...
Now we can start to explore the functions in 'HandyCNV'

## 1. How to prepare the standard cnv input file for 'HandyCNV' and got a fast summary?
We can use 'cnv_clean' function to convert cnv results to standard format. This function will also make a basic summaries of CNVs condition.<br> Let's formatting the cnv results from PennCNV at first.<br> 
Note: <br> 
a. If default results in PennCNV format, corresponding CNV input argument is 'penncnv'.<br> 
b. Default PennCNV result in Sample ID columns usually has path ahead the Sample ID, in example it's "/home/zjhang027/C/final_403_penncnv/201094560082_R03C01.txt", we can use 'cnv/'as separator in argument 'penn_id_sep = "cnv/"' to extarct Sample ID.

d. Set argument 'folder' will create a new folder in working directory to save results.
...
cnv_clean(penncnv = cnv_penn_ars, #when run your own data, set this argument by 'penncnv = "localpath/file"'
          penn_id_sep = "cnv/", 
          drop_length = 5, #used to remove the outlier CNVs by the limited Length, unit is Mb 
          folder = "cnv_clean") 
```

Then let's formatting the cnv results from CNVPartition, and save the results in the same folder.<br> 
Note:<br> 
CNV result from CNVPartition the corresponding input argument is 'cnvpartition'.
...
cnv_clean(cnvpartition = cnv_part_umd, #when run your own data, set this argument by 'cnvpartition = "localpath/file"' 
          drop_length = 5, # the maximum length threshold, unit is Mb, larger than this value the CNVs will be deleted 
          folder = "cnv_clean") 
```
If your cnv results not from PennCNV or Partition, you need to arrange your CNV results in to the standard format then use the 'standard_cnv' argument instead.<br> The standard cnv input list should include column by order with:<br> 'Sample_ID', "Chr", "Start", "End" and "CNV_Value".
...
Let's run 'cnv_clean' on the Standard CNV input file
```{r}
cnv_clean(standard_cnv = cnv_other_umd, #when run your own data, set this argument by 'standard = "localpath/file"'
          drop_length = 5, #used to remove the outlier CNVs by the limited Length, unit is Mb 
          folder = "cnv_clean")
```
...
cnv_visual(clean_cnv = "cnv_clean/penncnv_clean.cnv", 
...
Let's now to visualize CNVs from Part results. We can adjust the size of final figure by setting 'width_1' and 'height_1'.
```{r fig.show='hide', message=FALSE, warning=FALSE, echo=TRUE, results='hide'}
cnv_visual(clean_cnv = "cnv_clean/cnvpart_clean.cnv", 
           max_chr = 29, #select how many Chromosomes to plot
           width_1 = 20, #optional,adjust the width of final plot
           height_1 = 12, #optional,adjust the height of final plot
...
### 2.2 How to plot an interested region?
...
### 2.3 How to plot CNVs in an interested sample?
...
Note: The input file will generate in section 5.2. Play it after we annotated the gene to CNV results.
```{r fig.show='hide', warning=FALSE}
...
After we got the clean CNV results, we can use 'cnv_summary_plot' function to make summary plots. Which will plot CNV results by Length Group, CNV Type, Chromosome and Individuals, it's also could make two types combination plot from above figures.<br> First, let's plotting for Part_cnv result.
```{r cnv_summary_plot_penn, fig.show='hide', warning=FALSE}
cnv_summary_plot(clean_cnv = "cnv_clean/cnvpart_clean.cnv",
...
call_cnvr(clean_cnv = "cnv_clean/penncnv_clean.cnv", 
...
call_cnvr(clean_cnv = "cnv_clean/cnvpart_clean.cnv", 
          chr_set = '29', 
...
call_cnvr(clean_cnv = "cnv_clean/cleancnv.cnv", 
          chr_set = 29,
          folder = "call_cnvr_standard")
```

## 5. How to annotated genes for CNVR and CNV?
Once we got cnv and cnvr results, we might want to see how many genes located in these structure variation region? We can annotated gene to CNV and CNVR by 'call_gene' function. But we need to prepare the right reference gene lists before we calling genes. Now we have a problem, how do we get the reference genes? Let's move to next section to see the solution. 

### 5.1 How to download the reference gene lists we need?
Here we have a function 'get_refgene' could automatically download the reference gene and convert the it to the standard format. Let's see which version of reference gene we can download. To see the support information only need to run 'get_refgene' without any argument.
```{r}
get_refgene()
```
...
Note:In 'call_gene' function, there are three arguments, 'ref_gene' is essential, but 'interval' and 'clean_cnv' you can choose one of them or select all. We suggest to set both 'interval' and 'clean_cnv' simultaneously, then it will count and report the gene frequency which by counting how many CNVs overlapped to a gene within a CNVR. The term of 'interval' may confuse you, we call it as this because it not limited on CNVR list, it could be a CNV, QTL, Haplotype list or any other interval list with standard input format.<br> Now let's going to call genes for these structure variations.<br> 
...
call_gene(refgene = "refgene/cow_UMD_UCSC.txt", 
          ...
          clean_cnv = "cnv_clean/cnvpart_clean.cnv", 
          folder = "call_gene_Part")
```

Then let call gene for the third CNV results
```{r echo=TRUE, results='hide'}
call_gene(refgene = "refgene/cow_ARS_UCSC.txt", 
          interval = "call_cnvr_standard/cnvr.txt",
          clean_cnv = "cnv_clean/cleancnv.cnv", 
...
It always interested to see which genome region are most frequency to have the structure variation. We can plotting all the high frequent region by 'cnvr_plot' function.<br> Let's plot some for Penn Result at first. Remember to set the name of new folder to save results.
...
```{r, fig.show='hide'}
compare_cnv(cnv_def = "cnv_clean/cleancnv.cnv", 
            cnv_tar = "cnv_clean/penncnv_clean.cnv",
...
```{r, fig.show='hide'}
compare_cnvr(cnvr_def  = "call_cnvr_Part/cnvr.txt", 
             cnvr_tar  = "call_cnvr_standard/cnvr.txt", 
...
```{r, fig.show='hide'}
compare_gene(gene_freq_1 = "call_gene_Part/gene_freq_cnv.txt", #standard file was generated by 'call_gene' function
             gene_freq_2 = "call_gene_Penn_Ens/gene_freq_cnv.txt", #standard file was generated by 'call_gene' function
...
compare_gene(gene_freq_1 = "call_gene_Part/gene_freq_cnv.txt", 
             ...
             gene_freq_3 = "call_gene_standard_ARS_UCSC/gene_freq_cnv.txt",
...
compare_gene(gene_freq_1 = "call_gene_Part/gene_freq_cnv.txt", 
             ...
             gene_freq_4 = "call_gene_standard_ARS_UCSC/gene_freq_cnv.txt",
             title_1 = "Part", #optional, adjust the title of input file 1 present in final plot 
             title_2 = "ENS", #optional, adjust the title of input file 2 present in final plot 
             title_3 = "UCSC", #optional, adjust the title of input file 3 present in final plot 
...
convert_map(default_map = umd_map, #when run your own data, set this argument by 'default_map = "localpath/file"'
            ...
            col_2 = "red", #set color for the type of Unmatch in SNP comparison plot
...
plot_cnvr_source(cnvr = "call_cnvr_Penn/cnvr.txt", 
                clean_cnv = "cnv_clean/penncnv_clean.cnv", 
