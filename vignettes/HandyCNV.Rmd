...
```{r}
pedgree_test <- fread(system.file("extdata", "Demo_data/Pedigree/Pedigree.csv", package = "HandyCNV"))
plot_cnvr_source(cnvr = "call_cnvr_Penn/cnvr.txt", 
                ...
                Frequent_threshold = 60, height_1 = 30, width_1 = 15)
```
![Figure.20 Plot Source of CNVR](plot_cnv_source/Chr12_56.513728-64.796304Mb_Source.png){width=150px}

## 14. How to plot genes in a specific region and save it as figure seperatly?   
```{r, fig.show='hide'}
plot_gene(refgene = "refgene/cow_ARS_UCSC.txt", chr_id = 7, start = 11, end = 12, height_1 = 3, width_1 = 10)
```
![Figure.21 Gene plot](Chr7_11_12.png){width=300px}
## 15. How to find out the high frequency ROH region?
ROH (Run of homozygosity) is 
```{r}
roh_window(roh = "clean_cnv/cnvpart_roh.cnv", 
           window_size = 2,
...
```{r}
roh_visual(clean_roh = "clean_cnv/cnvpart_roh.cnv", 
           chr_id = 6, 
           width_1 = 14, 
           height_1 = 9, 
           folder = "roh_visual", 
...
```{r}
roh_visual(clean_roh = "clean_cnv/cnvpart_roh.cnv", chr_id = 6, start_position = 36.5, end_position = 40.6, width_1 = 16)
```
...
# 17. How to prepare the demo data from Siginal Intensity and Plink Genotype and Map file?
