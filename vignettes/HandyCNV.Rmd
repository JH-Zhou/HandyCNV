...
# Introduction
This package was originally designed for the Post-analysis of CNV results inferred from PennCNV and CNVPartition (GenomeStudio). But now it was expanded to accept the input files with standard formats for wider application. Our motivation is to provide a Standard, Reproducible and Time-saving pipelines for the post-analysis of CNV and ROH detected from SNP genotyping data for the most of diploid Species. All functions can be divide to five sectors: Conversion, Summary, Annotation, Comparison and Visualization. The most useful features are it could integrate summarize results, generate CNVR, annotated gene, plot CNVR distribution map and customize visualizing CNV and ROH with gene and other related information together. It's also support to adjust the color, size of high resolution figures, create folder to save results to avoid conflict between different results. Run through all functions the details reports could help us to explore the most interested genomic region easier. As following, We will present how to use these functions to solve the problems.

# Prerequisite
First, to run this package, we need to make sure the R (Version >= 3.5.2) was installed in your computer (R download link: https://www.r-project.org/).<br> second, install the 'HandyCNV' package from Github repository by following script:
```{r}
#library(remotes)
#install_github(repo = "JH-Zhou/HandyCNV", auth_token = "3d2ac98e4c297bab332f1e68b3b2d49f3a17d6aa")
```
Then, we need to library 'HandyCNV' by following code and use it later.
```{r setup}
library(HandyCNV)
```

To start play with this package, we need to prepare at least one CNV result list. With only CNV results as input file, we can explore the functions from section 1 to section 10 as below. But to get more interested information, we have to prepare more input files, including map file, signal intensity file, pedigree, and Plink format (bed, bim and fam) files. Don't afraid to prepare these files, most of them we already had and used in the CNV inferring progress. In order to get more valuable results, it worth to make the preparation, and after that, you will found the rest of work are much more easier and would saving lots of time. We will introducing how to prepare each input file during the relative chunk while using them.

# Demo data
We have provided some internal demo data and it should installed with package. We are going to take these data as example to present how to use this package.<br>

We can extract the demo data by 'system.file' (see as below). In demo data, the first CNV results is default output format from PennCNV with ARS-UCD 1.2 (ARS) reference genome; The second CNV results is defaults output from CNVPartition with UMD 3.1 (UMD) reference genome; The third CNV list is the standard input file could prepared from CNV results inferred by the other tools. <br> 

Note: If you are dislike to test with this demo data, just skip this step to section 1 and use your own data instead.
```{r}
...
As above we mentioned, the functions in the package are five sectors: Conversion, Summary, Annotation, Comparison and Visualization. Conversion part, could convert the map files between the maps file (default and target) provide by users. This is useful when we are detecting CNVs with two versions of reference genome, the converted map files has the well-prepared formats can used in Plink, PennCNV and our package 'HandyCNV'. Another feature is it support to convert coordinates for intervals (CNV, CNVR, QTL et al) between the defaults and target maps, which might useful when the users want to compare the results to others. Summarization part including format, details summary of CNV and ROH, make CNV summary plot, generate CNVRs and plot CNVR distribution map. Annotation part could auto-download reference gene lists from UCSC website and annotate genes for CNV, CNVR, ROH or other intervals. Comparison section including comparison of CNVs, CNVRs, Gene Frequent Lists and any other Intervals, the highlight is comparison of CNVs and CNVRs will reports details comparison results on both individual and population levels, and all comparison results are relative to both input files. Visualization part contains functions to support to customize CNV and ROH plot by Chromosome, specific Sample, interested regions or target genes (Plot by gene is not for ROH right now), we can also and add more information on each plots, such as annotated gene, Log R Ratio, B Allele Frequency, SNP genotype, LD or the source of CNVs. Besides, its can plotting genes separately from reference gene lists which might useful when we want to combine it with other plots.

#Setting up working directory
To start analysis, let's setup the working directory at first. Make sure all results will saving in the same directory.
```{r eval=FALSE}
...
## 1. How to prepare the standard cnv input file for 'HandyCNV' and get a fast summary?
We can use 'cnv_clean' function to convert cnv results to standard format. This function will also make brief summary of CNVs results.<br> Let's formatting the cnv results from PennCNV at first.<br> 
Note: <br> 
a. If default results in PennCNV format, corresponding CNV input argument is 'penncnv'.<br> 
b. Default PennCNV result in Sample ID columns usually has path ahead the Sample ID, in example it's "/home/zjhang027/C/final_403_penncnv/201094560082_R03C01.txt", the pattern 'cnv/' ahead sample ID is unique, we can take it as separator in argument 'penn_id_sep = "cnv/"' to extract Sample ID.
...
cnv_clean(penncnv = cnv_penn_ars, #when run your own data, set this argument by 'penncnv = "localpath/file"'
          penn_id_sep = "cnv/", #take the unique pattern as separator to extract the sample ID 
          drop_length = 5, #used to remove the outlier CNVs by the limited Length, unit is Mb 
          folder = "cnv_clean") #create new folder to save results 
```

Then let's formatting the second CNV results from CNVPartition.<br> 
...
cnv_clean(cnvpartition = cnv_part_umd, #when run your own data, set this argument by 'cnvpartition = "localpath/file"' 
          ...
          folder = "cnv_clean") 
```
If the CNV results are not detect by PennCNV or CNVPartition, then we need to arrange the CNV results by the standard format, then use the 'standard_cnv' argument instead.<br> The standard cnv input list should include columns by order with:<br> 'Sample_ID', "Chr", "Start", "End" and "CNV_Value".
...
Let's run 'cnv_clean' on the third Standard CNV input file. Pay attention here, we are using the argument 'standard_cnv' to the third CNV result.
```{r}
cnv_clean(standard_cnv = cnv_other_umd, #when run your own data, set this argument by 'standard_cnv = "localpath/file"'
          drop_length = 5, #used to remove the outlier CNVs by the limited Length, unit is Mb 
          folder = "cnv_clean")
```
